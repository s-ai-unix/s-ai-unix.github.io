...
7. **画布利用率（防止"图小背景大"）**
   - ❌ 错误：大边距导致内容挤在中间
     ```python
     # 不要这样设置，右边距400像素浪费空间
     margin=dict(l=80, r=400, t=100, b=80)
     ```
   - ✅ 正确：内容占满整个画布
     ```python
     # 小边距让内容填满画布
     margin=dict(l=50, r=50, t=80, b=50)
     # 时间线图使用全宽布局，避免内容集中在左侧
     range=[x_min - 2, x_max + 2]  # 时间线延伸到边缘
     ```
   - **时间线图特殊处理**：事件交错上下分布，充分利用垂直空间
     ```python
     # 时间线图最大化填充画布的完整配置
     # 1. 扩大Y轴范围，让事件延伸到画布边缘
     yaxis=dict(range=[-7.2, 7.2])  # 根据画布高度调整
     
     # 2. 事件位置分布到边缘（示例：10个事件交错分布）
     ...
     # 4. 时期标签也移到边缘位置
     fig.add_annotation(x=1740, y=6.5, text='经典时期', ...)  # 靠近上边缘
     fig.add_annotation(x=1875, y=-6.5, text='黎曼几何时期', ...)  # 靠近下边缘
     ```
   - **最大化填充 checklist**：
     - [ ] Y轴范围扩大到接近边距极限
     - [ ] 事件/节点位置分布到画布上下边缘
     - [ ] 标签/注释位置相应调整
     - [ ] 减少 margin 到最小必要值（确保文字不被截断的前提下）
     - [ ] 检查所有文字可见，不被边缘截断

8. **对比图设计规范**（多子图对比时）
   
   当使用多子图（`make_subplots`）展示对比概念时，**每个子图必须有独特的视觉特征**：
   
   | 区分手段 | 应用场景 | 示例 |
   |---------|---------|------|
   | **背景色块** | 标记特殊区间 | `add_vrect()` 添加 dx 区间 |
   | **箭头方向** | 表示动态过程 | 左图红色"趋近"箭头 |
   | **标记形状** | 标识关键点 | 菱形标记极限点 |
   | **线型差异** | 区分边界线 | 虚线表示 epsilon/delta |
   | **填充区域** | 表示误差范围 | 橙色填充 epsilon 带 |
   
   **正例**（极限概念三种理解）：
   - 左图：红色箭头（趋近）+ 绿色菱形（极限值）→ 动态直观
   - 中图：紫色背景（dx 区间）+ 公式标注 → 无穷小概念
   - 右图：橙色水平线（ε）+ 绿色垂直线（δ）+ 填充区域 → 严格定量
   
   **反例**（避免）：
   ```python
   # ❌ 错误：三图几乎相同，只有标题不同
   for i in range(1, 4):
       fig.add_trace(go.Scatter(x=x, y=y, line=dict(color='#007AFF')), row=1, col=i)
...
**解决**：检查上标格式，使用 `^{...}` 而非 `^(...)`，并避免 `^*` 这种特殊字符上标

**常见错误示例**（星号上标）：
| 错误写法 | 正确写法 | 说明 |
|---------|---------|------|
| `\delta^*` | `\delta^{\ast}` | 估计量上标 |
| `\phi^*` | `\phi^{\ast}` | 检验函数上标 |
| `\theta^*` | `\theta^{\ast}` | 参数估计上标 |
| `^*` 单独使用 | `^{\ast}` | 必须包裹在花括号内 |

- 原因：某些 MathJax 配置中，`*` 作为上标可能被解析为特殊符号，导致 "Missing open brace" 错误
- 通用规则：所有上标都使用 `^{...}` 格式，即使只有一个字符

**症状**：希腊字母或特殊符号显示异常

**解决**：使用 LaTeX 命令（`\alpha`, `\sigma`）而非 Unicode 字符

**症状**：`\boldsymbol` 命令无法识别（如 `\boldsymbol{\theta}`）

**解决**：MathJax 默认配置可能不支持 `\boldsymbol`，应使用普通形式
- ❌ 错误：`\boldsymbol{\theta}`（需要 AMSmath 扩展）
- ✅ 正确：`\theta`（标准形式，兼容所有配置）
- 注意：向量可用 `\mathbf{x}`（拉丁字母）或普通希腊字母表示
