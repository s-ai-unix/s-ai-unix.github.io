...
**症状**：浏览器控制台报错 "Double exponent"

**解决**：双重上标必须用括号明确，如 `$(\mathbf{v}'_{w})^T$` 而非 `$\mathbf{v}'_{w}^T$`

**症状**：希腊字母或特殊符号显示异常

**解决**：使用 LaTeX 命令（`\alpha`, `\sigma`）而非 Unicode 字符

**症状**：图注中的公式渲染错误，显示 `_图：` 或 `*图：`

**解决**：图注避免使用 `*...*` 或 `_..._` 包裹含公式的文本。改用 `<p class="caption">图：纯文本描述</p>` 或去掉公式仅用文字说明。

详见 [LATEX-MATH.md](references/LATEX-MATH.md) 的"常见错误"和"图注/图片标题中的公式特殊处理"章节。
