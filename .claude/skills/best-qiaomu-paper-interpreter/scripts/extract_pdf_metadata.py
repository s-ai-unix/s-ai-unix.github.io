...
def organize_paper_directory(pdf_path, output_base="01.inbox/papers", url=None, user_hint=None):
    """
    组织论文目录结构
...
def main():
    ...
    if len(sys.argv) < 2:
        print("用法: python extract_pdf_metadata.py <PDF文件> <输出目录> [URL] [用户标识]")
        print("示例: python extract_pdf_metadata.py paper.pdf papers https://arxiv.org/pdf/1910.10683")
        print("      python extract_pdf_metadata.py bert.pdf papers \"\" BERT")
        sys.exit(1)

    pdf_path = sys.argv[1]
    output_base = sys.argv[2] if len(sys.argv) > 2 else "01.inbox/papers"
    url = sys.argv[3] if len(sys.argv) > 3 and sys.argv[3] else None
    user_hint = sys.argv[4] if len(sys.argv) > 4 else None
