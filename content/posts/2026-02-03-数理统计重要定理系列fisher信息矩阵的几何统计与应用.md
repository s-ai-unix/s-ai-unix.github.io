...
设 $\boldsymbol{\theta} = (\theta_1, \ldots, \theta_p)^T$ 是 $p$ 维参数向量。对数似然函数 $\ell(\boldsymbol{\theta})$ 是 $\boldsymbol{\theta}$ 的标量函数。

**得分向量**（score vector）是对数似然的梯度：

<div class="math">
$$
\mathbf{S}(\boldsymbol{\theta}) = \nabla_{\boldsymbol{\theta}} \ell(\boldsymbol{\theta}) = \left(\frac{\partial \ell}{\partial \theta_1}, \ldots, \frac{\partial \ell}{\partial \theta_p}\right)^T
$$
</div>
...
<div class="math">
$$
\mathcal{I}(\boldsymbol{\theta}) = \mathbb{E}\left[\mathbf{S}(\boldsymbol{\theta}) \mathbf{S}(\boldsymbol{\theta})^T\right]
$$
</div>
...
<div class="math">
$$
\mathcal{I}(\boldsymbol{\theta})_{ij} = \mathbb{E}\left[\frac{\partial \ell}{\partial \theta_i} \frac{\partial \ell}{\partial \theta_j}\right]
$$
</div>
...
<div class="math">
$$
\mathcal{I}(\boldsymbol{\theta})_{ij} = -\mathbb{E}\left[\frac{\partial^2 \ell}{\partial \theta_i \partial \theta_j}\right]
$$
</div>
...
&= -\mathcal{I}(\boldsymbol{\theta})_{ij} + \frac{\partial^2}{\partial \theta_i \partial \theta_j} \int L \, dx \\
&= -\mathcal{I}(\boldsymbol{\theta})_{ij} + 0 = -\mathcal{I}(\boldsymbol{\theta})_{ij}
\end{align}
$$
</div>
...
对于i.i.d.样本，$\ell(\boldsymbol{\theta}) = \sum_{k=1}^n \log f(X_k; \boldsymbol{\theta})$，因此：

<div class="math">
$$
\mathcal{I}_n(\boldsymbol{\theta}) = n \cdot \mathcal{I}_1(\boldsymbol{\theta})
$$
</div>
...
考虑一个参数化的概率分布族 $\{P_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \Theta\}$。例如，正态分布族 $N(\mu, \sigma^2)$ 由两个参数 $(\mu, \sigma^2)$ 索引。

这个分布族可以看作一个**统计流形**（statistical manifold）：每个点对应一个概率分布，参数空间提供了局部坐标。

**关键问题**：在这个流形上，如何定义"距离"？两个分布 $P_{\boldsymbol{\theta}}$ 和 $P_{\boldsymbol{\theta} + d\boldsymbol{\theta}}$ 有多"接近"？
...
<div class="math">
$$
ds^2 = \sum_{i,j} \mathcal{I}(\boldsymbol{\theta})_{ij} \, d\theta_i \, d\theta_j = d\boldsymbol{\theta}^T \mathcal{I}(\boldsymbol{\theta}) d\boldsymbol{\theta}
$$
</div>
...
4. **体积元**：$\sqrt{\det \mathcal{I}(\boldsymbol{\theta})} d\boldsymbol{\theta}$ 定义了参数空间上的自然体积元，与Jeffreys先验有关。
...
**多元Cramér-Rao下界**：设 $\hat{\boldsymbol{\theta}}$ 是 $\boldsymbol{\theta}$ 的无偏估计，则：
...
这意味着 $\text{Cov}(\hat{\boldsymbol{\theta}}) - \mathcal{I}(\boldsymbol{\theta})^{-1}$ 是半正定矩阵。
...
<div class="math">
$$
[\mathcal{I}(\boldsymbol{\theta})^{-1}]_{ii} \geq \frac{1}{\mathcal{I}(\boldsymbol{\theta})_{ii}}
$$
</div>

等号成立当且仅当 $\theta_i$ 与其他参数"正交"（即 $\mathcal{I}(\boldsymbol{\theta})_{ij} = 0$ 对所有 $j \neq i$）。
...
考虑线性模型 $\mathbf{y} = \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\epsilon}$，其中 $\boldsymbol{\epsilon} \sim N(\mathbf{0}, \sigma^2 \mathbf{I})$。
...
其中 $\mathbf{T}(x)$ 是充分统计量，$A(\boldsymbol{\theta})$ 是对数配分函数。
...
对于 $N(\boldsymbol{\mu}, \boldsymbol{\Sigma})$，Fisher信息矩阵与协方差矩阵的逆密切相关。这解释了为什么在高斯模型中，精度矩阵（协方差矩阵的逆）在推断中起核心作用。
...
<div class="math">
$$
\mathcal{J}(\boldsymbol{\theta})_{ij} = -\frac{\partial^2 \ell(\boldsymbol{\theta})}{\partial \theta_i \partial \theta_j}
$$
</div>

这与**期望Fisher信息矩阵**（expected Fisher information）$\mathcal{I}(\boldsymbol{\theta})$ 不同：前者基于实际观测的数据，后者是理论期望。

在MLE $\hat{\boldsymbol{\theta}}$ 处，观测信息矩阵 $\mathcal{J}(\hat{\boldsymbol{\theta}})$ 常用于：
- 计算标准误：$\widehat{\text{SE}}(\hat{\theta}_i) = \sqrt{[\mathcal{J}(\hat{\boldsymbol{\theta}})^{-1}]_{ii}}$
- 构造置信区间
- 似然比检验的近似
...
在变分推断中，我们用简单的分布 $q(\mathbf{z}; \boldsymbol{\theta})$ 逼近复杂的后验分布 $p(\mathbf{z} | \mathbf{x})$。优化目标是最小化KL散度：

<div class="math">
$$
\min_{\boldsymbol{\theta}} \text{KL}(q(\mathbf{z}; \boldsymbol{\theta}) || p(\mathbf{z} | \mathbf{x}))
$$
</div>
...
- **D-最优**：最大化 $\det \mathcal{I}(\boldsymbol{\theta})$（置信椭球的体积最小）
- **A-最优**：最小化 $\text{tr}(\mathcal{I}(\boldsymbol{\theta})^{-1})$（参数方差之和最小）
- **E-最优**：最大化 $\lambda_{\min}(\mathcal{I}(\boldsymbol{\theta}))$（最坏情况最好）
...
1. **Fisher信息矩阵** $\mathcal{I}(\boldsymbol{\theta})$ 量化了数据包含的关于多维参数的信息，其元素由得分向量的协方差给出。
