...
给定数据 $x_1, \ldots, x_n$ 来自真实分布 $P_{\text{data}}$，考虑参数化模型族 $\{P_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \Theta\}$。

**最大似然估计**（MLE）最大化对数似然：

<div class="math">
$$
\hat{\boldsymbol{\theta}}_{\text{MLE}} = \arg\max_{\boldsymbol{\theta}} \frac{1}{n} \sum_{i=1}^n \log p_{\boldsymbol{\theta}}(x_i)
$$
</div>
...
<div class="math">
$$
\frac{1}{n} \sum_{i=1}^n \log p_{\boldsymbol{\theta}}(x_i) \to \mathbb{E}_{P_{\text{data}}}[\log p_{\boldsymbol{\theta}}(X)]
$$
</div>
...
<div class="math">
$$\begin{align}
\hat{\boldsymbol{\theta}}_{\text{MLE}} &= \arg\max_{\boldsymbol{\theta}} \mathbb{E}_{P_{\text{data}}}[\log p_{\boldsymbol{\theta}}(X)] \\
&= \arg\min_{\boldsymbol{\theta}} \left(-\mathbb{E}_{P_{\text{data}}}[\log p_{\boldsymbol{\theta}}(X)]\right) \\
&= \arg\min_{\boldsymbol{\theta}} D_{KL}(P_{\text{data}} || P_{\boldsymbol{\theta}})
\end{align}
$$
</div>
...
但对于复杂模型，后验难以计算。**变分推断**（Variational Inference）用一个简单的分布 $q(\boldsymbol{\theta})$ 来近似后验，通过最小化KL散度：
...
<div class="math">
$$\begin{align}
D_{KL}(q || p(\cdot | \mathbf{x})) &= \mathbb{E}_q[\log q(\boldsymbol{\theta})] - \mathbb{E}_q[\log p(\boldsymbol{\theta} | \mathbf{x})] \\
&= \mathbb{E}_q[\log q(\boldsymbol{\theta})] - \mathbb{E}_q[\log p(\mathbf{x} | \boldsymbol{\theta}) + \log p(\boldsymbol{\theta})] + \log p(\mathbf{x}) \\
&= -\text{ELBO}(q) + \text{const}
\end{align}
$$
</div>

其中 $\text{ELBO}(q) = \mathbb{E}_q[\log p(\mathbf{x} | \boldsymbol{\theta})] - D_{KL}(q || p(\boldsymbol{\theta}))$ 是**证据下界**（Evidence Lower BOund）。最小化KL散度等价于最大化ELBO。
...
在第三章中，我们介绍了Fisher信息矩阵 $\mathcal{I}(\boldsymbol{\theta})$。KL散度与Fisher信息之间存在深刻的关系。

考虑参数空间中的两个邻近点 $\boldsymbol{\theta}$ 和 $\boldsymbol{\theta} + d\boldsymbol{\theta}$。对 $D_{KL}(P_{\boldsymbol{\theta}} || P_{\boldsymbol{\theta} + d\boldsymbol{\theta}})$ 进行二阶泰勒展开：

<div class="math">
$$
D_{KL}(P_{\boldsymbol{\theta}} || P_{\boldsymbol{\theta} + d\boldsymbol{\theta}}) = \frac{1}{2} d\boldsymbol{\theta}^T \mathcal{I}(\boldsymbol{\theta}) d\boldsymbol{\theta} + o(\|d\boldsymbol{\theta}\|^2)
$$
</div>
...
3. **统计意义**：MLE等价于最小化 $D_{KL}(P_{\text{data}} || P_{\boldsymbol{\theta}})$；假设检验中，KL散度决定错误指数的衰减速率。
