...
数学上，流形 $M$ 上每一点 $p$ 都有一个对应的切空间 $T_p M$ 。这个切空间是一个向量空间，里面的元素称为**切向量**，代表在该点可能的速度方向。

$$
T_p M = \text{所有经过点 } p \text{ 的曲线的速度向量}
$$
...
$$
\nabla_{\mu} R^{\nu}_{\sigma} - \nabla_{\nu} R^{\mu}_{\sigma} + \nabla_{\lambda} R^{\lambda}_{\sigma\mu\nu} = 0
$$
