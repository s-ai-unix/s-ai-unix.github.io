...
蒙日-安培方程（Monge–Ampère equation）是非线性偏微分方程理论中最核心的对象之一。它以“行列式”形式约束 Hessian 矩阵，体现了曲率、体积变换与最优传输之间的深刻联系。在几何、最优传输、流体动力学、光学设计以及现代机器学习中，它都扮演着结构性角色。

本文从历史背景出发，梳理方程的发展与理论演进，并给出核心公式的推导与应用场景，为系统理解该方程提供一条完整路径。

## 历史背景与演进

### 1. 蒙日的最优传输起源（1781）
蒙日在研究土方运输问题时提出“以最小功搬运质量”的问题，这就是最优传输的原型。它在现代以 Monge 问题的形式出现：寻找一个映射将一个质量分布搬运到另一个分布，并最小化代价。

### 2. 安培与几何扩展（1820）
安培进一步研究曲面与几何结构中的二阶非线性方程，给出了与 Monge 工作相呼应的形式，该类方程因此被命名为 Monge–Ampère 方程。

### 3. 20 世纪：弱解与正则性理论
- **Minkowski / Weyl 问题**：凸几何中的曲率处方问题与 Monge–Ampère 方程紧密相连。
- **Alexandrov（1940s）**：引入 Monge–Ampère 测度与 Alexandrov 弱解概念，使得非光滑情形得以研究。
- **Calabi / Pogorelov（1950s–1970s）**：建立内点估计与正则性理论，明确了凸性在椭圆性中的关键地位。
- **Cheng–Yau、Lions（1970s–1980s）**：发展 Dirichlet 问题的经典解方法。

### 4. 最优传输的复兴与 Caffarelli 理论（1980s–1990s）
Brenier 证明了二次代价下的最优传输映射为凸函数的梯度，直接导出 Monge–Ampère 方程。Caffarelli 随后建立了 $W^{2,p}$ 和 $C^{2,\alpha}$ 等强正则性估计，使得该方程理论体系完整成型。

### 5. 现代发展（2000s–至今）
Monge–Ampère 方程在几何光学、气象学、机器学习（如生成模型与密度映射）中得到大量应用，同时数值方法和非光滑理论也持续发展。
