...
**对偶空间**：所有1-形式构成的集合称为**余切空间**，记作 $T_p^* M$。它与切空间 $T_p M$ 是"对偶"关系。
...
**拉回（Pullback，记作 $f^*$）**：把 $N$ 上的1-形式拉到 $M$ 上
...
1. **右等变**：对于 $g \in GL(n, \mathbb{R})$，有 $R_g^* \omega = g^{-1} \omega g$
...
1. **右等变**：$R_g^* \omega = \text{Ad}_{g^{-1}} \circ \omega$
